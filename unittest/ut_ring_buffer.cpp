...
TEST_F(ut_ring_buffer, resize) {
    ring.resize(8);
    EXPECT_EQ(8, ring.capacity());

    ring.emplace(1);
    EXPECT_NO_THROW(ring.resize(8));      // OK, no change
    EXPECT_ANY_THROW(ring.resize(16));    // Not OK
    EXPECT_ANY_THROW(ring.resize(0));     // Not OK
}
